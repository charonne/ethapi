...
method.deploy = function (transaction) {
    ...
    } else if (transaction.tx_hash != null) {
        // Get informations
        method.getInformations(transaction, function (err, resp) {
            console.log(resp);
        });
    }
}
...
method.verify = function (transaction, callback) {
    ...
    var now = new Date();
    var timeoutAt = new Date(transaction.processedAt);
    timeoutAt.setMinutes(timeoutAt.getMinutes() + config.ethereum.timeout);
    // Is timeout
    if (now >= timeoutAt) {
        // Try do another attempt
        if (transaction.attempt <= config.ethereum.attempt) {
            // Retry
            Transaction.update({'_id': transaction.id}, {'$set': {
                    'processed': 0, 'processedAt': null, 'tx_hash': null, 'status': 0
                }}, function(err, resp) {
...
method.process = function () {
    ...
    Transaction.find({status: 0})
        ...
        transactions.forEach(function(transaction) {
            ...
            logger.log("info", 'Process: ' + transaction.type + ', tx: ' + transaction.id);
