...
method.getInfos = function() {
    ...
    logger.log("info", "------------ Infos (" + new Date() +") ------------");
 ...
 * Unlock account
 */
method.unlock = function() {
    // Unlock
    var unlock = personal.unlockAccount(this.account_address, this.account_password);
...
method.create = function(transaction, callback) {
    logger.log("info", "Create contract");
    ...
    contractData['callback'] = 
        function(err, newcontract) {
            if(!err) {
                logger.log("info", "executed");
                if(!newcontract.address) {
                    logger.log("info", "tx: " + newcontract.transactionHash) // The hash of the transaction, which deploys the contract
                    // Set transaction
                    Transaction.update({'_id': transaction.id},
                        {'$set': {'tx_hash': newcontract.transactionHash, 'processed': 1, 'processedAt': new Date()}},
                        { '$inc': { attempt: 1 } }, 
                        function(err, resp) {
...
method.exec = function(transaction, callback) {
    logger.log("info", "Exec contract address: " + transaction.contract_address);
    ...
    contractData['callback'] = 
        function(err, tx_hash) {
            ...
            // Set transaction
            Transaction.update({'_id': transaction.id},
                {'$set': {'tx_hash': tx_hash, 'processed': 1, 'processedAt': new Date()}},
                { '$inc': { attempt: 1 } },
                function(err, resp) {
